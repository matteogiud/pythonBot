#from ...telegram.models.telegram_users import DbTelgramUser
import sys
import os
...
def execute_insert_dict(
    table_name: str,
    data: list,
    table_values_names: tuple,
    dict_values_names: tuple,
    db_table_log=None,
):
    ...
    with Database.lock:
        for item in data:
            values_str = ", ".join(
                [str(item.get(field)) for field in dict_values_names]
            )
            insert_query = (
                f"INSERT INTO {table_name} ({field_str}) VALUES ({placeholders_str})"
            )
...
def get_closer_gas_station_from_db(latitude: float, longitude: float, user, limit: int = 5):
    import sqlite3
    ...
    with Database.lock:
        # user_cursor = conn.execute(
        #     "SELECT *, ROUND(2 * 6371 * ASIN( SQRT( POWER( SIN((? - abs(latitudine)) * pi() / 180 / 2), 2) + COS(? * pi() / 180) * COS(abs(latitudine) * pi() / 180) * POWER( SIN((? - longitudine) * pi() / 180 / 2), 2))), 2) AS distance FROM installations ORDER BY distance LIMIT ?",
        #     (latitude, latitude, longitude, limit)
        # )
        stations_cursor = conn.execute(
            """SELECT *, ROUTEDISTANCE(latitudine, longitudine, ?, ?) AS route_distance 
                FROM (SELECT *, ROUND(2 * 6371 * ASIN( SQRT( POWER( SIN((? - abs(latitudine)) * pi() / 180 / 2), 2) + COS(? * pi() / 180) * COS(abs(latitudine) * pi() / 180) * POWER( SIN((? - longitudine) * pi() / 180 / 2), 2))), 2) AS distance 
                        FROM installations JOIN fuel_prices ON installations.id = fuel_prices.idImpianto
                        WHERE tipoCarburante = ?
                        ORDER BY distance
                        LIMIT ?)
                ORDER BY route_distance 
                LIMIT ?""",
            (latitude, longitude, latitude, latitude, longitude, user.DbCar.fuel_type, limit * 2, limit),
        )
    rows = stations_cursor.fetchall()

    column_names = [description[0] for description in stations_cursor.description]
...
def get_cheaper_gas_station_from_db(
    latitude: float,
    longitude: float,
    user,
    lt_to_do: int,
        ...
        stations_cursor = conn.execute(
            """SELECT *, ROUTEDISTANCE(latitudine, longitudine, ?, ?) AS route_distance 
                FROM (SELECT *, ROUND(2 * 6371 * ASIN( SQRT( POWER( SIN((? - abs(latitudine)) * pi() / 180 / 2), 2) + COS(? * pi() / 180) * COS(abs(latitudine) * pi() / 180) * POWER( SIN((? - longitudine) * pi() / 180 / 2), 2))), 2) AS distance 
                        FROM installations JOIN fuel_prices ON installations.id = fuel_prices.idImpianto 
                        WHERE tipoCarburante = ?
                        ORDER BY distance, prezzoSelf
                        LIMIT ?)
                ORDER BY CHEAPRATIO(route_distance, prezzoSelf, ?, ?) ASC
                LIMIT ?""",
            (
                latitude,
                longitude,
...
def set_fuel_type_in_db(car_id: int, fuel_type: str):
    ...
    with Database.lock:
        res = conn.execute(
            "UPDATE cars SET tipo_carburante = ? WHERE id = ?", (fuel_type, car_id)
        )
        conn.commit()
...
def set_capacity_in_db(car_id: int, capacity: str):
    ...
    with Database.lock:
        res = conn.execute(
            "UPDATE cars SET capacita_serbatoio = ? WHERE id = ?", (capacity, car_id)
        )
        conn.commit()
...
def set_consume_in_db(car_id: int, consume: str):
    ...
    with Database.lock:
        res = conn.execute(
            "UPDATE cars SET consumo_km_per_lt = ? WHERE id = ?", (consume, car_id)
        )
        conn.commit()
