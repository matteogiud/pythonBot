...
def load_db_from_local_files(delete_table_first=True):
    ...
    execute_insert_dict(
        "fuel_prices",
        fuel_price_dict,
        ("idImpianto", "tipoCarburante", "prezzoSelf"),
...
def clean_fuel_price_dict(fuel_price_dict: list) -> list:
    def get_fuel_type(fuel_type: str) -> str:
        ...
        elif fuel_type.lower().find("dies") != -1 or fuel_type.lower().find("gaso") != -1:
...
def clean_installations_dict(installations_dict: list) -> list:
    ...
    for item in installations_dict:
        installation = {
            ...
            "Nome Impianto": None
            if str(item["Nome Impianto"]).lower() == "null"
            else item["Nome Impianto"],
            "Bandiera": None
            if str(item["Bandiera"]).lower() == "null"
            else item["Bandiera"],
            "Indirizzo": None
            if str(item["Indirizzo"]).lower() == "null"
            else item["Indirizzo"],
            "Comune": None if str(item["Comune"]).lower() == "null" else item["Comune"],
            "Provincia": None
            if (str(item["Provincia"]).lower() == "null" or len(item["Provincia"]) > 2)
            else item["Provincia"],
        }
...
def check_csv_updated():
    ...
    try:
        with open(f"{downloading_folder}fuel_price.csv", "r") as file:
            first_line = file.readline()

            # Utilizza regex per cercare il pattern corrispondente alla data
